...
`gunzip /var/tmp/mobyDick.txt.gz` if -e  "/var/tmp/mobyDick.txt.gz";
...
`sort -R /var/tmp/mobyDick.txt  | sed 's/,//g' | awk -F" " '{ print \$1, \$2, \$3;}' | sed 's/-//g' | egrep -v 'AND\$' | grep -v '^AND' | grep -v '^"AND' > /var/tmp/mobyRandom.csv`;  # Randomize it every time -- combatting Solr's caching efforts
